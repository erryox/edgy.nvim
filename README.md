...
- **edgebar** windows can not be **resized** like normal windows by default.
  ...
  persist window sizes changed by dragging a border with the mouse. This
  only takes effect for actual mouse drags (detected via `vim.on_key()`),
  not typed `:resize` commands, so it can't be fooled by unrelated windows
  opening/closing or the editor being resized.
...
```lua
{
  ...
  -- Allow persisting window sizes changed by dragging a border with the
  -- mouse, instead of always snapping back to the configured size.
  -- Off by default (see the Limitations section).
  mouse_resize = {
    enabled = false,
  },
